...
import tempfile
import os
from cotan.Injector import inject_cotan, prepare_get_samples, after_get_samples
...
def get_peft_model(model, config, args=None, tokenizer=None, logs=None):
    if config.init_strategy in  ["corda", "cotan", "cotan_svf", "cotan_half"]:
    ...
    return inject_cotan(config, model, logs)
