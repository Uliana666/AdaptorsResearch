...
class CoTAnLinear(nn.Module):
    ...
    def reset_parameters(self, base_tensor):
        ...
        elif self.init_strategy == "cotan":
            B, A = Compressors.COTAN(base_tensor, self.X, self.r)
            del self.X
            self.adapter_A.copy_(A.T)
            self.adapter_B.copy_(B.T)
            
        elif self.init_strategy == "cotan_svf":
            B, A = Compressors.COTAN(base_tensor, self.X, self.r)
            del self.X
            self.adapter_A.copy_(A.T)
            self.adapter_B.copy_(B.T)
        ...
        elif self.init_strategy == "cotan_half":
            B, A = Compressors.COTAN_HALF(base_tensor, self.X, self.r)
            del self.X
            self.adapter_A.copy_(A.T)
            self.adapter_B.copy_(B.T)
        else:
    ...
    def get_value(self):
        if self.init_strategy != "cotan_svf":
